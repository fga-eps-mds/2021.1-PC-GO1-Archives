# Generated by Django 3.2.6 on 2021-09-09 23:43

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='BoxAbbreviations',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('number', models.IntegerField(blank=True)),
                ('abbreviation', models.CharField(blank=True, max_length=20)),
                ('name', models.CharField(blank=True, max_length=100)),
                ('year', models.IntegerField(blank=True)),
            ],
        ),
        migrations.CreateModel(
            name='Document',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('process_number', models.CharField(max_length=100)),
                ('sender_unity', models.CharField(max_length=100)),
                ('notes', models.CharField(max_length=300)),
                ('abbreviation_id', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.boxabbreviations')),
            ],
        ),
        migrations.CreateModel(
            name='DocumentSubject',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('subject_name', models.CharField(blank=True, max_length=100)),
                ('temporality', models.DateField(blank=True, null=True)),
            ],
        ),
        migrations.CreateModel(
            name='DocumentType',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('document_name', models.CharField(blank=True, max_length=100)),
                ('temporality', models.DateField(blank=True, null=True)),
            ],
        ),
        migrations.CreateModel(
            name='FrontCover',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('box_abbreviation', models.CharField(blank=True, max_length=30)),
            ],
        ),
        migrations.CreateModel(
            name='OriginBox',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('number', models.CharField(max_length=100)),
                ('year', models.IntegerField()),
                ('subject', models.CharField(max_length=150)),
                ('date', models.DateField()),
            ],
        ),
        migrations.CreateModel(
            name='PublicWorker',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(blank=True, max_length=100)),
                ('cpf', models.CharField(blank=True, max_length=11)),
                ('office', models.CharField(blank=True, max_length=100)),
                ('class_worker', models.CharField(blank=True, max_length=100)),
                ('capacity', models.CharField(blank=True, max_length=100)),
                ('county', models.CharField(blank=True, max_length=100)),
            ],
        ),
        migrations.CreateModel(
            name='Shelf',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('shelfe_number', models.IntegerField(blank=True)),
                ('shelfp_number', models.IntegerField(blank=True)),
            ],
        ),
        migrations.CreateModel(
            name='Status',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('filed', models.BooleanField(blank=True)),
                ('eliminated', models.BooleanField(blank=True)),
                ('sent_from', models.CharField(blank=True, max_length=100)),
                ('requested_document', models.CharField(blank=True, max_length=100)),
                ('send_date', models.DateField(blank=True, null=True)),
            ],
        ),
        migrations.CreateModel(
            name='Unity',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('unity_name', models.CharField(blank=True, max_length=100)),
                ('unity_abbreviation', models.CharField(blank=True, max_length=20)),
                ('administrative_bond', models.CharField(blank=True, max_length=100)),
                ('bond_abbreviation', models.CharField(blank=True, max_length=20)),
                ('type_of_unity', models.CharField(blank=True, max_length=30)),
                ('municipality', models.CharField(blank=True, max_length=100)),
                ('telephone_number', models.CharField(blank=True, max_length=8)),
                ('note', models.CharField(blank=True, max_length=100)),
            ],
        ),
        migrations.CreateModel(
            name='Relation',
            fields=[
                ('document_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='archives_app.document')),
                ('number', models.CharField(max_length=1000)),
                ('received_date', models.DateField()),
                ('document_type', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.documenttype')),
            ],
            bases=('archives_app.document',),
        ),
        migrations.CreateModel(
            name='FrequencySheet',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('due_date', models.DateField()),
                ('notes', models.CharField(max_length=300)),
                ('process_number', models.CharField(max_length=1000)),
                ('abbreviation_id', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.boxabbreviations')),
                ('public_worker_id', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.publicworker')),
                ('shelf_id', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.shelf')),
            ],
        ),
        migrations.AddField(
            model_name='document',
            name='shelf_id',
            field=models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.shelf'),
        ),
        migrations.CreateModel(
            name='AdministrativeProcess',
            fields=[
                ('document_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='archives_app.document')),
                ('notice_date', models.DateField()),
                ('interested', models.CharField(max_length=150)),
                ('cpf_cnpj', models.CharField(max_length=15)),
                ('reference_month_year', models.FloatField()),
                ('archiving_date', models.DateField()),
                ('dest_unity', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.unity')),
                ('sender_worker', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.publicworker')),
                ('status', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.status')),
                ('subject', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.documentsubject')),
            ],
            bases=('archives_app.document',),
        ),
        migrations.CreateModel(
            name='FrequencyRelation',
            fields=[
                ('relation_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='archives_app.relation')),
                ('due_date', models.DateField()),
                ('frequency_receiver_id', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.publicworker')),
            ],
            bases=('archives_app.relation',),
        ),
        migrations.CreateModel(
            name='ArchivalRelation',
            fields=[
                ('relation_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='archives_app.relation')),
                ('number_of_boxes', models.IntegerField()),
                ('document_url', models.URLField()),
                ('cover_sheet', models.CharField(max_length=100)),
                ('box_receiver', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, to='archives_app.publicworker')),
                ('origin_box_id', models.ManyToManyField(to='archives_app.OriginBox')),
            ],
            bases=('archives_app.relation',),
        ),
    ]
